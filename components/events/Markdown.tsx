...
import { eventImageVariants, firstMarkdownImageSrc, parseEventImageSrc } from "@/lib/events/markdown-image";
...
//  · img 주소(v1.61): 원본 대신 폭별 변환본(srcSet · sizes, WebP) — 원본 PNG 2.2MB 가 폭 750 에서 약 61KB.
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          img: ({ src, alt }) => {
            ...
            // 원본 대신 폭별 변환본(v1.61) — 변환할 수 없는 주소면 원본 그대로.
            const sources = eventImageVariants(url, size) ?? { src: url };
            // 크기를 알면 width/height 만 단다 — 표시는 종전과 같다(원래 크기, 칸보다 크면 칸 폭 · height:auto).
            if (size) {
              // eslint-disable-next-line @next/next/no-img-element
              return <img {...sources} alt={alt ?? ""} width={size.width} height={size.height} loading={loading} />;
            }
            return (
              <span className="block aspect-[40/21] w-full">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img {...sources} alt={alt ?? ""} loading={loading} className="h-full w-full object-contain" />
              </span>
            );
          },
        }}
