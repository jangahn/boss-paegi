...
// 무PII: 식별자/서비스 내 원본 URL/query/IP 미저장. 클라 값 불신 — sanitize(core) + member_state 서버 판정.
// v1.31: UA 원문은 서버가 헤더에서 읽어 행에 얹고(withUserAgent), 외부 레퍼러 원문은 sanitize 가 검증해 visit 행에 남긴다.
...
export async function POST(req: NextRequest) {
  // 봇 게이트(v1.08) — 크롤러 UA·무UA(비브라우저 클라이언트) 드롭. 통과한 UA 원문은 v1.31 부터 행에 저장한다.
  const userAgent = req.headers.get("user-agent");
  if (!isTrackableUserAgent(userAgent)) return noContent();
  if (!originAllowed(req)) return noContent();
  const raw = await readTrackJsonRequest(req);
  if (raw === null) return noContent();

  const sanitized = sanitizeTrackPayload(raw);
  if (!sanitized) return noContent();
  const row = withUserAgent(sanitized, userAgent);
