...
export async function POST(req: NextRequest) {
  ...
  const refresh = legacyAdminClientRefresh("integrityVoid", body);
  if (refresh) {
  ...
  if (error) {
    log.warn("admin.integrity.void_fail", {
  ...
  if (!result) {
    log.error("admin.integrity.void_invalid_result", { scoreId: body.scoreId });
    return NextResponse.json({ error: "action_failed" }, { status: 500 });
  }
  revalidatePath("/leaderboard");
  log.info("admin.integrity.void_ok", {
    scoreId: body.scoreId,
