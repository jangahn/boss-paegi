...
2. **AI 생성 프롬프트·수치는 `generation_config`(어드민 콘텐츠 콘솔) 소유.** 기본 시드 = 캐릭터화 템플릿(chibi super-deformed / plush felt / white background / identity 보존)이며 어드민이 편집·롤백 가능. 강제 키워드를 **코드로 강제하지 않으며**(사용자 결정), 초상권·명예훼손 억제는 버전 이력·롤백·어드민 검토로 관리한다. 프롬프트 텍스트는 코드에 리터럴로 두지 말고 config·`assembleGenerationPrompts`(`lib/config/domains/generation.ts`)를 단일 소스로 쓴다.
