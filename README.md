...
- **무기 뱃지 tier** `[2,4,6,8,9]` → `[2,4,6,8,10,13,16,19]`(도달 상한 19종): `weapon_9` 는 slug 동결 규약대로 **코드 은퇴(active=false 고정, 획득 표시 보존)**, 새 tier 는 발행 카탈로그에 없어도 읽기·쓰기 정규화가 편입(additive, `SEED_BADGES`·`CODE_RETIRED_BADGE_SLUGS`; **v1.38 부터 `Seed.added` 표시 tier 만**). 맵 뱃지 `[2,3,4,5,6]` 그대로.
