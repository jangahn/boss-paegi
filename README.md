...
- **cron 백스톱**(`/api/ops/integrity-scan`, `supabase/migrations/0053`·`0054`): 텔레메트리는 delta 스트리밍이라 제출시점 미확정 가능 → 최근(6h) `registered` 점수를 **확정 텔레메트리**와 대조해 사후 flag. 신호 **C1**(score 20%+ 불일치)·**C1B**(duration 20%+ 불일치)·**C2**(세션 apm>1200 & ≥60s)·**C8**(연결 텔레 `suspicious`). **C1/C1B 정합 검사는 `tscore>0` 그리고 완주 텔레(`end_reason ∈ normal/time_limit/score_limit`)일 때만**(0054): 빈/abandoned/**hidden_timeout 절단** 텔레는 duration·score가 부분값이라 정상 점수를 오탐(전수조사 2026-07-08: C1B 발화 100%가 hidden_timeout 절단 아티팩트). `registered→pending` 만, idempotent(score_flags PK). `x-cron-secret`. **⚠ cron-job.org 등록 필요**(미등록 시 제출시점 신호만 작동).
