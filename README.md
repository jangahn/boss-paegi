...
- **앱 표면**: 어드민 `/admin/refunds`(운영 큐)·회원 상세 로트/환불 개시(RefundButton)·대시보드 4범주 경고 / 사용자 마이페이지 2탭(`/account` 회원정보 · `/account/payments` 결제내역 — 결제완료 주문만·최신순·결제수단/테스트/잔여크레딧·영수증·KST 시분). 크레딧 개수는 헤더 드롭다운(AccountMenu). 탈퇴용 `GET /api/account/refundable-credits`. 생성 lifecycle 원자화(`create_generation_and_consume`·`create_generation_row`·`mark_generation_failed_and_refund`).
