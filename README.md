...
- **꼬질꼬질 데미지 데칼** (`DamageLayer`): **1,000점마다** 약한 꼬질 (때 + 작은 멍/스크래치), **10,000점마다** 눈에 띄는 큰/진한 멍 + 넓은 얼룩 — 상한 없이 계속 누적 (성능 안전망: 데칼 400개 초과 시 오래된 것부터 정리). 위치는 랜덤이 아니라 **피격 부위 기준** — 모든 타격의 좌표를 기록해 최근 맞은 곳 부근에 쌓임 (실루엣 밖이면 재시도 → 랜덤 fallback). 낙서처럼 인형과 함께 움직이고 라운드 리셋 시 초기화. zustand subscribe 로 리렌더 없이 점수 전달. 자연스러움: 멍 = 불규칙 폴리곤 + radial gradient, 때 = 가우시안 스프레이, 스크래치 = 휜 곡선
