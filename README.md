...
- **로그인은 자유, 동의는 글로벌 게이트(렌더 전)**: 별도 가입 페이지 없음 — `/login` 버튼뿐. OAuth 로그인하면 세션은 확립되지만, **로그인했는데 미동의면 `proxy.ts` 가 모든(비예외) 페이지 진입을 렌더 전에 `/consent` 로** 막는다(뒤로가기·직접URL·클라내비 우회 불가). 동의 화면 선택지는 **[동의] 또는 [로그아웃]뿐**. **회원 생성·OAuth 닉/프사 시드·가입보너스·익명이전은 모두 동의 시점**(`/api/account/consent` → `create_or_update_member_consent` RPC, row 없으면 INSERT)에 일어난다 → 동의 전 처리 최소화. 콜백은 회원을 만들지 않고 동의여부만 판정해 미동의면 직접 `/consent`, 동의완료면 목적지로. `/consent` 의 약관/방침 "보기"는 인라인 모달(전문 로드 실패 시 동의 차단). 예외 경로 = `/consent`·`/auth/*`·`/api/*` + 정적/`_next`(`lib/routes.isConsentExempt`); **`/login` 은 anon 전용**(로그인 사용자가 가면 미동의→`/consent`·완료→`/`). `/signup`·`/reconsent` 는 `/consent` redirect stub. **회원탈퇴는 동의완료 회원만 도달**(`account/delete`=`requireAuthedNonDeleted`, 계정 종료 권리).
- **proxy 글로벌 게이트(`proxy.ts`)**: GET/HEAD 문서/RSC 내비에만(POST/Server Action 은 endpoint `requireMember` 백스톱 — 앱에 server action 없음). 로그인 사용자는 `profiles.deleted_at`+`member_accounts.{age,terms,privacy}` self-read(`maybeSingle`, no-row≠실패) + edge-safe 현재버전(`lib/legal/edge-versions`, isolate별 60s 캐시) → `lib/consent.missingConsentItems` 단일 규칙으로 판정. **실패 정책**: member 조회 실패=fail-closed(`/consent`), 버전 조회 실패=fail-open(stamp 회원 안 잠금, age만), `deleted_at` set=auth 쿠키 만료+`/login?error=account_deleted`(잔존 세션 루프 방지). redirect 는 세션 쿠키 보존+`no-store`. 결제 webhook(`/api/pay/webhook`)은 updateSession 전 즉시 pass-through. 약관/방침 **개정** 시 기존 동의자도 60s 내 모든 페이지에서 재동의(action 은 `requireMember` 즉시). `missingConsentItems` 는 `<` 비교(동의 버전이 현재보다 높아도 OK — 두 버전 소스 publish 직후 divergence 루프 방지). **⚠️ 법적**: 서비스 사용(생성·결제) 전 동의 강제로 우려 대부분 해소 — OAuth 이메일이 로그인 시 auth.users 에 저장되는 것만 잔존(OAuth 본질, 경미). 약관/방침 문서 반영 과제.
...
- **마이그레이션(익명→회원)**: 익명 상태 OAuth 로그인 시 **서명 쿠키(`MIGRATE_COOKIE`, `lib/cookies`)** 로 익명 데이터(dolls/scores)를 **동의 시점 신규 회원 INSERT(is_new) 1회에만** 이전(`reassign_anon_data`, best-effort). 콜백은 미동의면 MIGRATE 유지(consent API 가 이전+clear), 동의완료면 clear. **기존 회원 재로그인은 병합 안 함**(보수적). RPC 성공 시 항상 clear(migrate 실패도 Sentry+clear). 로그아웃(`/api/auth/signout`: server `auth.signOut`+sb-* 쿠키+MIGRATE 만료)·타계정 로그인 시 오이전 방지. `identity_already_exists`→`/login`.
...
- **생성권(크레딧)**: 가입 시 2개 지급(`member_accounts.gen_credits`), 생성마다 1개 차감(서버 `consume_gen_credit`, fal 제출 직전 원자적·실패 시 `refund_gen_credit`). 소진 시 `/credits` 에서 **유료 충전**(포트원 결제, 아래 *결제* 참조) 또는 의견 위젯 요청. `OPS_USER_ID` 는 무제한.
...
- **paymentId**: 가맹점 채번 = `order_uuid` 하이픈 제거 hex(**KPN 이 영숫자만 허용**). 같은 paymentId 는 포트원이 "성공 1회만" 보장(중복 결제 차단) — 10분 내 같은 user+product pending 재사용과 결합해 중복 주문 방지.
...
- **PG 심사용 노출**: 심사·테스트 계정 판정은 `lib/reviewer.ts isReviewerUser` 단일 소스 — ① `growth_levers.reviewerEmails`(콘솔 성장 레버 "테스트 결제 계정 이메일", OAuth 심사관 allowlist) ② `reviewer_accounts`(0060, `/admin/reviewers` 에서 ID/PW 계정 CUD — 생성 시 auth email 유저+동의 스탬프+원장 insert, 비번은 응답 1회 표시). `creditsEnabled=false`(전역 준비중)여도 심사 계정에겐 /credits 결제 UI·체크아웃 허용. ID/PW 진입은 `/login?reviewer=1`(평상시 로그인 UI 불변). email 가입 계정은 가입보너스 0(공개 email 프로바이더 경유 보너스 파밍 차단). 심사 계정 주문은 테스트 채널 + `is_test`로 기록되어 어드민 매출·KPI 에서 제외(주문 목록·회원 상세·환불 경고에 TEST 뱃지 표시). **사업자정보 푸터**(`business_info.info` — 콘솔 '사업자 정보' 탭, v0.74 에 site_content 에서 분리)는 심사 요건(상호·사업자번호·대표자·주소·유선전화 상시 노출).
...
- **동시/다중 결제**: 같은 상품 동시 checkout 은 10분 내 pending 재사용 + 버튼 disable + user 당 rate-limit(10/분). 미완료 pending 은 대사가 정리(미결제 이탈=failed 종단).
...
- **멀티 라우트**(공통 `app/admin/layout.tsx` = `requireAdmin` 1회 + `AppNav`+`AdminNav`): `/admin`(대시보드) · `/admin/orders`(전체 주문) · `/admin/refunds`(환불 운영 큐, v0.76) · `/admin/users`(회원) · `/admin/ledger`(처리내역) · `/admin/generations`(캐릭터 생성) · `/admin/moderation`(신고) · `/admin/integrity`(무결성 — 어뷰징 리뷰 큐) · `/admin/events`(이벤트/소식 — 공지·이벤트 작성/발행·홈 팝업·배너 운영, v0.50) · `/admin/reviewers`(PG 심사·테스트 계정, v0.73) · `/admin/content`(콘텐츠) · `/admin/analytics`(게임 분석) · `/admin/acquisition`(공유·유입 — 게임 분석과 격리). **회원 표기 = 회원 상세 링크(v0.74)**: 어드민에서 회원(닉네임/이메일/id)을 표기하는 모든 표면은 `/admin/users/[id]` 로 링크한다(관리자 컬럼·집계 화면 제외) — 신규 표면 추가 시 이 규약 준수.
...
- **운영 액션**(돈·감사): stuck **포트원 검증 후 지급**(서버가 단건 조회로 PAID·금액 확인 후에만 RPC) · 정상결제 **환불**(paid 전용·포트원 자동취소+회수, 부족 시 차단) · **pending 취소**(`/api/admin/cancel` → 포트원 취소 연동, 미결제 pending 은 로컬 표시만; 회수 없음) · CS 크레딧 조정(회원만·−100~100·≠0·사유 5~500). service_role RPC(`admin_settle_stuck_order`/`admin_cancel_order`[5-arg + 4-arg wrapper, 무중단]/`admin_adjust_credits`)가 **row lock→변경→`admin_actions_ledger` 기록**을 한 트랜잭션(멱등·취소 1회·화해·pg_done 시 clamp+shortfall).
...
- **서버 판정·fail-closed 제출**(`lib/anti-abuse-rules.ts`, `supabase/migrations/0051`): `/api/score` 가 protocol 검증 → 신호 판정 → **원자 RPC `submit_score_with_review`**(scores+score_flags 한 트랜잭션, service-role only, integrity 실패 시 롤백=조작 점수가 registered 로 안 샘) → `registered` 에만 페르소나/백분위/뱃지. 제출 신호(버전 `ANTI_ABUSE_RULES_VERSION`, 임계는 튜닝 상수 파생): **S1** 지속 타격속도(>18/s@60s+·>25/s), **S2** 무기별 fresh(300) 차감 avg/hit>실효 이론상한×1.05(최소 20타 — 실효 max base 는 속도 배율 상한 반영: swipe ×2.0·throw ×2.2·grab fling strength+30, 고정무기는 strength; v2 교정 2026-07-03), **S3** score/초>1400, **S4** stats 누락/`validateGameplayStats` 실패, **S6** notable(≥30만) 무텔레메트리, **S7** duration>15분 AND score>126만(15분 봉투 상수 — 장기 세션의 봉투 연장, v5 교정 2026-07-13; 텔레 duration 대조는 제출시점 미확정이라 cron C1b 전담), **S8** 연결 텔레 `suspicious`(단조), **S10** ultScore>궁극 가능횟수×상한(ultScore 몰아넣기 폐쇄). 하나라도 발화→`pending`(공개면 숨김·운영자 검토), banned 유저→`voided`. 하드 유저리젝 없음(공개면 숨김만).
- **제출 UX**(`components/ScoreReport`·`GameOverModal`·`useScoreSubmission`·`marketing_copy`): flagged(pending/voided) 점수는 종료화면에 "⏳ 랭킹 검토 중" 안내 + 정지 경고(마케터 편집 `share.pendingReviewNotice`/`pendingReviewWarning`) 표시, 공유 버튼·뱃지·페르소나 미노출. 정상은 기존과 동일.
...
- **펜**: PNG 알파맵 기반 캐릭터 실루엣 판정 — 실루엣 밖 낙서 불가 (보간 dot 까지 재검증). 낙서는 doll.bodyWrap 의 child — 캐릭터가 흔들리거나 던져지면 낙서도 같은 레이어로 함께 이동
- **배경 전환 시 게임 상태 유지**: BgSwitcher 가 navigation 대신 텍스처 핫스왑 (`setBackground`) — 점수/콤보/무기/낙서 전부 유지, URL 은 replaceState 동기화
- 안정성: PIXI v8 globalpointermove 로 fling 추적 (캐릭터 밖 드래그 추적 유지), touch-action none 유지 (모바일 제스처 하이재킹 방지), DOM pointercancel 안전망, 물리 body 반경 표시 scale 동기화, collisionStart 넉백 setVelocity 임펄스화, fling 중 중력 누적 방지, 게임 생성 중 무기/배경 변경 유실 방지
...
- **계정 탈퇴(soft-delete)**: 셀프 즉시(`profiles.deleted_at`), **`auth.users` 삭제 안 함**(삭제 시 결제기록 CASCADE 파괴). 캐릭터 이미지 storage 삭제+`dolls` row 삭제·`display_name`="탈퇴한 사용자"·`avatar_url`/`member_accounts.email`=null·`gen_credits`=0, `scores` 비식별 보존. `auth.users` 이메일 스크럽(best-effort). 재로그인 차단(`requireMember`·auth callback이 `deleted_at` 가드, upsert 전). UI=AccountMenu "계정 삭제" 2-step 모달. `/api/account/delete`(same-origin·pending 결제 409·DB 먼저·storage list+remove best-effort).
...
- **만14세 게이트**: "미만 이용 불가"(DOB·보호자동의 미구현). `member_accounts.age_confirmed_at` 1회 플래그(`/api/account/confirm-age` 멱등). 로그인 14세 체크 + 생성(ConsentDialog age-14)·결제 서버 게이트(`age_required` 403→클라 확인 후 재시도). 익명 미게이트.
- 문서: 시드 draft 최소 갱신(탈퇴 처리·결제 5년·14세 미만 불가·크레딧 소멸). **후속 과제(미구현·문서 미약속)**: 별도 감사 테이블·자동환불·법정대리인 동의 플로우. (신고/모더레이션·콘텐츠 자동삭제 cron·운영자 강제삭제는 **v0.38/Migration 0034** 에서 구현.)
...
- **신고 창구**: 공개 `POST /api/report`(**인증 불요** — 피해자는 보통 비가입 제3자), `/doll`·`/share`·`/history` 🚩버튼(doll 있을 때만 노출). 사유 allowlist·rate-limit(IP+target 인메모리)·dedup(target pending 시 알림 skip). 신고 1건당 **Sentry 알림**(`report.new` — 자동숨김 없으므로 알림이 SLA, 어드민 deep link 동봉, 인증 없는 one-click 토큰 없음). 오프플랫폼 접수=공개 보호책임자 이메일(인프라 0).
...
- **인접 cron**(`app/api/ops/content-maintain`, `x-cron-secret`=`CRON_SECRET`, cron-job.org 등록): 만료 하이라이트 purge·고아 `tmp/face` sweep(30분+)·`artifacts_purged_at` 미확정 doll 물리삭제 백스톱.
...
- **읽기 전 표면 signed URL**: 헬퍼 `lib/storage-path.ts`(순수 `dollPath`·client 안전) + `lib/storage.ts`(server-only `signedDollUrl`/`signedHighlightUrl`, ttl 인자). 서버: doll page/OG·share page/OG·history 상세·score-detail(clip)·admin(모더레이션/회원). 클라: **`POST /api/doll/signed-urls`**(id 로 DB 조회한 path 만 서명·UUID·ids≤50·deleted 제외·no-store·IP rate-limit·`createSignedUrls` 배치) → useGameInit(ttl 3600)·gallery(50청크·실패 시 기본 보스 강등)·`/api/generations`(후보 우리버킷/fal 분기 서명·ttl 6h). write(`api/doll`·`generation-recovery`)는 경로 저장, 삭제 핸들러(doll DELETE·account/delete)는 `dollPath`.
- **복구·영구삭제 어드민**: `/api/admin/moderation/{restore,permanent-delete}`. restore=이 takedown 이 숨긴 하이라이트만 되살림(`highlight_deleted_by_doll` 태깅·만료 등 다른 숨김 불간섭). permanent-delete=artifact purge(storage 객체 제거·dolls row 보존), `deleted_at not null`+`artifacts_purged_at null` 가드, **전부 성공만 purged**·실패 path 추적. 모더레이션 큐에 [복구][영구삭제] 버튼(purged 면 "복구 불가"). takedown 라우트는 물리삭제 제거(가역) + `revalidateDollSurfaces`(doll/share/history+OG 무효화). cron 자동 purge 백스톱 제거.
...
- 검증: typecheck 0·build 0. `<AppNav` 렌더 grep=root+admin 2곳뿐(라우트 상호배타 → double-nav 0). SSR fetch 로 라우트별 노출 일치(홈·faq·랭킹·약관 노출 / play·login·signup·consent·reconsent 미노출) + 홈 단일 nav preview 확인. 남은 후속: 갤러리 서명URL 클라 캐싱(④).
...
- 검증: typecheck 0·build 0. **후속**: ②제출전 얼굴검출(PR-1b)·②webhook 서버회수(PR-2)·진행률 UX+사진안내 카피(PR-3, reason:photo 소비).

v0.60 (2026-06-29, 제출 전 얼굴 게이트 #1-b — no-face 즉시 반려; 마이그레이션 없음):
- **`lib/fal.ts`**: `detectGlasses` → `analyzeInputFace`(VLM **1회** 호출로 얼굴 존재+안경 동시 판별, 추가 비용 0). `'face=no'` 명시 시에만 반려, 모호·파싱실패·예외는 통과(**fail-open** — 정상 사진 과반려 방지, PR-1 no-face 즉시실패+환불이 안전망).
...
- **클라 폴링 의존 제거**: 탭 닫힘·앱 백그라운드로 폴링이 멈추면 fal 완료돼도 결과를 못 채워 좀비가 되던 것(+result 만료 시 영구 손실)을, 클라와 무관한 **cron 스윕**으로 해소. (webhook 대신 — ED25519 서명검증·로컬 테스트 불가 회피, 동일 목표를 기존 로직 재사용으로 안전하게.)
...
- 검증: typecheck 0·build 0. **⚠ 배포 후: cron-job.org 에 `POST https://boss-paegi.vercel.app/api/ops/gen-recover` (`x-cron-secret` 헤더 = CRON_SECRET) 5분 주기 등록 — 미등록 시 좀비 회수 안 됨**(클라 폴링만으로 동작은 하나 이탈/만료 손실 잔존).
...
- **PG 심사 대응**: `growth_levers.reviewerEmails`(콘솔) — creditsEnabled OFF 여도 심사 계정에만 /credits 결제 UI·체크아웃 허용(`creditsAllowedFor` 단일 판정, /credits 는 서버 페이지화). `site_content.businessInfo`(콘솔) + 전역 `SiteFooter`(사업자정보 상시 노출 — 심사 요건, /play 등 게임 표면 제외). 테스트/실연동 전환=채널키 env 교체만.
...
- **심사계정 카피 정정**: 성장 레버 "PG 심사용 계정 이메일" → **"테스트 결제 계정 이메일"** — 실의미가 2중임을 명시(①등록 계정 결제는 creditsEnabled 와 무관하게 항상 테스트 채널 기본, `?live=1` 시만 실채널 ②OFF 여도 /credits 결제 UI·체크아웃 허용) + ID/PW 계정은 `/admin/reviewers` 별도 관리(OR 판정, 같은 효력) 상호참조. '심사 종료 후 비우세요' → 유지 운영(런북 §7 정책)으로 정정. reviewers 페이지·런북의 구 명칭 참조 동시 갱신.
...
- **시스템 반영은 후속 별도 설계**(약관 확정 우선 — 운영자 결정): 필요 공백 = 유·무상 버킷(단일 `gen_credits` 풀 분리+가입보너스 기록)·무상 우선 차감 RPC·포트원 부분취소(`amount` — API 지원 확인됨)·부분 회수 RPC(주문당 1회 유니크 인덱스·전량 회수 해제)·만료 로직(첫 만료 2027-07 전). 현재 실결제 0건·유료 잔액 보유자 0명이라 약관-시스템 불일치 리스크 없음, 그동안 환불 요청은 수동 절차(포트원 콘솔 부분취소 + CS 조정 회수).
...
- **입력 검증 재작성(2026-07-27)**: ⚠️ 기존 "5판정 1콜 compound 프롬프트"는 **무력**이었음 — moondream3/query 가 복수질문을 **첫 질문만 답하고 무시**(`face:yes` 만 반환) → 나머지 parse 전부 fail-open → 모든 사진 통과(브이·여러명 뚫림). fal 실측 후 **체크별 단일질문 병렬 호출**로 재작성(`lib/character-gen/face-analysis.ts` 순수 판정 + `lib/fal.ts` 병렬 fal 콜): 얼굴 유무 · **인원 수(여러 명)** · **가림(손/물건)** · 안경. 제출·차감 전 반려 `no_face`/`multiple_people`/`face_obstructed`(각 원인별 재촬영 안내, 명시 위반만 차단=fail-open). **정수리 잘림은 입력 반려에서 제외** — moondream 이 이상적 증명사진까지 오반려해 신뢰성 미달(실측), 완결성은 생성 프롬프트(headTemplate·negative)가 담당(사용자 결정). provenance `analyze` 에 faceVisible/singlePerson/peopleCount/faceClear/wearsGlasses + 체크별 프롬프트·raw 기록(구 레코드는 optional 로 back-compat), 어드민 상세에 인원수·가림·검증 원문 표시. 판정 단위 테스트 `__tests__/generation/face-analysis.test.ts`.
- **입력 거부 어드민 가시화 + 거부 분류 확장(2026-07-27)**: 입력 거부는 원래 row 생성 **전** 400 이라 `ai_generations` 에 안 남아 어드민 불가시였음(실측 입력거부 row 0). 이제 거부 시 `create_generation_row`(소비 없음)→`status=failed`+`fail_reason`+`gen_params(analyze+rejected)` 기록 → 어드민 "거부" 카테고리에 표시(크레딧 미차감, `credit_lot_id=NULL` 이라 재환불 sweep·좀비백스톱과 무간섭·G-1 불변). 거부 분류를 `no_face` 단일 → `INPUT_REJECT_REASONS`(no_face/multiple_people/face_obstructed, `face-analysis.ts` 단일정본, route 게이트↔어드민 분류 공유) 로 확장. 라벨 `거부(얼굴X)`→`거부(입력 부적합)`, 사유 한글화, `toCreditNote` 거부=미차감 교정. provenance `config`/`generation` optional(거부 row 는 없음)+`rejected`. **QA 적재 매트릭스**: picked/done/queued·post-row 실패(fail_reason)·입력거부=적재 ✓; insufficient_credits(402, 소비前 롤백)·pre-row 예외(genId 없음, Sentry)만 미적재(의도). failed+NULL 33건=legacy(≤2026-06-29). 테스트 `__tests__/generation/provenance.test.ts`.
...
- **탈퇴·원본 얼굴 정리 fail-closed**: `/api/account/delete`는 soft-delete RPC 전에 dolls·highlight뿐 아니라 본인 업로드 `profiles.avatar_url` 경로도 수집한다. RPC 후 candidate/tmp-face/highlight/avatar Storage와 auth 식별정보를 모두 정리하되 Supabase SDK가 resolve하는 `{ error }`까지 검사하며, 독립 정리는 끝까지 시도한 뒤 하나라도 실패하면 500+오류 로그로 끝내 성공 응답/`account.delete_success`를 내보내지 않는다. `deleteFaceTmp`와 signed URL 실패 보상삭제도 Storage `remove()`의 `{ error }`를 throw/경고한다. 외부 호출 없는 fault-injection 단위 테스트로 Storage list/remove·Auth resolved-error 분기를 고정한다.
