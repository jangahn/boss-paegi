...
- **대시보드**(`/admin/analytics`, AdminNav '게임 분석' 탭, [오늘|7일|30일|전체] v1.06 하이브리드 — 오늘=`telemetry_rollup_rows_for_day` 라이브·과거=`telemetry_rollups` `day_kst<오늘`): **무기 편중·다양성**(단일무기율·메인무기분포·tap카테고리·세션평균집중도) · **무기 효율·파워**(메인무기 점수/초 중앙값 — 히스토그램 근사) · **맵 고착·전환**(단일맵율·시작맵분포·전환율) — 이 셋도 0110 `sess_*` dim 으로 롤업을 탄다. + 무기·맵 밸런스(타격·점수 비중·갭 — 빈도 교란되는 "점/타" 폐기) · 펀널·이탈 · 회원 활동(코호트·재방문 — 익명 ephemeral, **raw 직조회 예외**: 윈도우 간 회원 distinct 는 일단위 분해 불가) · 세션 인스펙터(`/admin/analytics/sessions/[id]` 타임라인 재생, raw). '전체' 의 세션단위 지표는 0110 백필 이전 과거가 잔존 세션 근사(각주 고지). 차트 라이브러리 없이 CSS 바. `lib/admin-analytics.ts`(+순수 수학 `lib/admin-analytics-math.ts`).
