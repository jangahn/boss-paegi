...
- **`score_config`(PR4)**: 점수 10단계 등급 라벨/한 줄 평(=&apos;패기 유형&apos;). **tier 매핑·간격(step)은 코드 고정**, 라벨 텍스트만 라이브 편집(Zod `length(10)`). `gradeFor(score, grades?)` + `ScoreConfigProvider`(클라)/`getScoreConfig()`(서버 share·history). 편집=`/admin/content/score_config`. step 조절·과거결과 동결은 후속(PR5 play_sessions 와 함께).
