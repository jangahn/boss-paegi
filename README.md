...
- **PC 전용 표시**(`pointer-fine:` — 모바일 레이아웃 불변): 무기 버튼 숫자 배지·맵 버튼 Q~Y 배지·궁극기 버튼 Space 배지(+`aria-keyshortcuts`), 무기에 마우스를 올리면 안내 캡슐이 그 무기의 키보드 조작법("스페이스: 랜덤 부위 타격 · 방향키: 그 방향 부위 타격" — 무기 이름은 넣지 않는다)으로 바뀐다.
