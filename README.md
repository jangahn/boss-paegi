...
- **화면(두 계층, 사용자 결정)**: 대시보드 '가입·구매 퍼널' → ①**유저 퍼널** = 기간 내 활동 유저 5단계(방문·플레이·가입·생성·결제) 가로 카드 5장(구 FunnelStep 토큰 + 방문·플레이 회원 수 줄, 전환율은 이전 단계 대비 숫자만) ②**유저 구성** = 단계별 처음/다시 카드 2장 행(라벨 + Stat, 다시엔 전체 대비 %; 가입은 다시 없음(—), 결제의 다시=재구매). 표 한 장·카드 한 장(단계별 4줄) 안은 한눈에 안 들어온다는 피드백으로 분리. 게임 분석 '회원 활동(재방문(2회+))' → **회원 플레이 빈도**(회원 세션·플레이 회원·**2세션+ 회원** — 세션=게임 한 판, '재방문'은 대시보드 '다시'에만), '플레이내 펀널'→'퍼널'(표기 통일). 공유·유입 '방문 유입 현황'→'방문 출처(탭 세션 단위 · 현재 진입)', 전환표 캡션에 플레이=첫 점수 제출·가입=신규 회원 명시.
