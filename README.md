...
- **화면**: 대시보드 '가입·구매 퍼널' → **유저 퍼널·구성** 표(overflow-x-auto, 375px 규칙). 방문·플레이 칸에 회원 수 작은 글씨 병기, 전환율은 전체 행 기준 이전 단계 대비, 가입은 '다시' 없음(—), 결제는 재구매가 '다시'. 게임 분석 '회원 활동(재방문(2회+))' → **회원 플레이 빈도**(회원 세션·플레이 회원·**2세션+ 회원** — 세션=게임 한 판, '재방문'은 대시보드 '다시'에만), '플레이내 펀널'→'퍼널'(표기 통일). 공유·유입 '방문 유입 현황'→'방문 출처(탭 세션 단위 · 현재 진입)', 전환표 캡션에 플레이=첫 점수 제출·가입=신규 회원 명시.
