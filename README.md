...
- **뱃지 튜닝**(후속): 캡 **30분/500만점**(`score-limits` MAX_DURATION_MS/MAX_SCORE_HARD — DB check 1h/10M 보다 타이트, 마이그레이션 불요). 카탈로그 **60개**로 — 점수 1,000~1,000,000(10), 궁극기 1~50회(10), 플레이타임 1~20분(10); 타격/콤보/무기/맵 현행. 종료화면 CTA 에 "🏅 내 뱃지"(/badges) 랜딩. BadgeChallenge HUD **컴팩트**(폭 88px·숫자 제거 progress bar 만 → 원래 면적 ~30%, 소형 폰서 캐릭터 비가림).
