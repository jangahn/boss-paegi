...
- **랜덤 닉네임 v2**(0128): 종전 "접두어 15 + 4자리 숫자"(평균 10.2자) → **"접두 수식어명사" 3조각**(접두 20·수식어 40·명사 50 = 40,000 조합, 각 조각 ≤3자·공백 1개 → 6~10자, 숫자 없음). 활성 프로필과 겹치면 30회 재추첨. 자동 패턴 그대로인 활성 프로필(익명 2,088 + 자동 이름 회원 3)을 **백필**(직접 바꾼 닉네임은 유지), 이전 이름은 `nickname_backfill_2026_09`(RLS·권한 차단)에 보관. `__tests__/account/nickname-vocab.test.ts` 가 어휘·길이·패턴을 고정.
