...
- **CI 하네스 플레이크 수정**(`scripts/qa/test-public-write-quota-races.sh`): 같은 세션 경합에서 뒤 연결을 `sleep 0.05` 간격으로 출발시키던 것을, 앞 연결이 잠금을 잡은 것을 `pg_stat_activity` 표시로 본 뒤 출발하게(`wait_for_activity`, anon-reassign 하네스와 같은 방식). 앞 연결의 잠금 유지는 0.2초 → 0.1초 — `ingest_telemetry_delta` 의 `lock_timeout` 250ms 에 느린 러너 지연이 더해지면 뒤 연결이 `quota_busy` 로 포기해 아무것도 세지 않았다(PR #314 CI 1|1:1|1:1). 전역 행 잠금 경합도 같은 방식으로 순서를 보장.
