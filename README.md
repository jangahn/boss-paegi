...
- **캐릭터 목록·상세**(`/admin/dolls`, `/admin/dolls/[id]`, 「캐릭터 생성」 메뉴의 「캐릭터」 탭 — v1.29): 살아 있는 결과물의 현재 상태(롤·성별·상태 칩·회원)와 **롤·성별 제어(어드민 전용, 유저 불가)**·변경 이력·생성 기록 링크·신고 링크. 생성 기록(`/admin/generations`)은 과정의 불변 기록. 링크 규약: 캐릭터→캐릭터 상세, 생성→생성 상세, 회원→회원 상세.
