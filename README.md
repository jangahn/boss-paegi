...
- **텔레메트리 거절 진단(`app/api/telemetry/route.ts`, `lib/telemetry/ua-family.ts`)**: `telemetry.identity_rejected` 에 `hasAuthCookie`(존재 여부만)·`contentType`·`beacon`(이탈 sendBeacon 은 `?beacon=1` 표식, `lib/telemetry/transport.ts`)·`uaFamily`(저카디널리티 계열) 추가. 같은 세션의 다른 전송은 수락되는데 이탈 순간 1건만 세션 없이 도착하던 사례(9/3 세션 292 중 10)의 원인 확정용 — 동작 변화 없음.
