...
/** 인간 실측 최대 score/초 — 2026-09-11 5손가락·투척 9종·맵 6·44초 정상 플레이 130,130점(어드민 정상 확인).
 *  v1.36 멀티터치 투척·맵변경 배율 ×4 시절 값(그 전 단일 터치 시절 1,267). 어드민 상세 라벨·S3 유도의 기준. */
export const HUMAN_SCORE_PER_SEC_OBSERVED = 2_947;
...
/** 최대 평균 score/초(S3 의심 플래그) = 인간 실측 최대 × 1.15 여유(2,947 × 1.15 ≈ 3,389 → 3,400; v11).
 *  v1.36 때 1,400→2,800 비례 상향 후 "2주 뒤 재측정" 예정이었는데, 배포 당일 5손가락 정상 플레이가 2,947 로 초과해 재측정.
 *  ⚠ 봉투 계층 불변식: `MAX_AVG_SCORE_PER_SEC`(4000, score-limits.ts 저장 하드상한) ≥ 이 값(3400, 의심
 *  플래그) ≥ `HUMAN_SCORE_PER_SEC_OBSERVED`(2947). 상한은 "저장 거부"용(정상 안 막게 넉넉), S3 는 "의심→리뷰"용이라
 *  서로 다른 계층 — 같게 두지 말 것. 상한을 낮추면 정상 고득점을 거부하고, S3 를 올리면 봇을 놓친다.
 *  텔레메트리 적재 RPC 의 suspicious 비율 임계는 S3 가 아니라 저장 상한(4000)과 같다(0130 — S8 은 "프로토콜상 불가능"만). */
export const SCORE_PER_SEC_MAX = 3_400;
/** S7 점수 하한 = S3 의 15분 봉투 상수(SCORE_PER_SEC_MAX × 900s = 3,060,000; v10 2,520,000·v1.35 까지 1,260,000).
