 ...
 * 궁극기를 위조하던 여유치를 제거한다.
 */
export const ANTI_ABUSE_RULES_VERSION = "2026-09-anti-abuse-v13"; // v13: 제한 시간 판 실측(초당 3,748) → S3 3400→4500 · 저장 봉투 4000→5500(0133) · S11 은 S3 유도(120초 562,500) · v12: S11 제한 시간 점수 상한(S3 × (최대 플레이 시간 + 5초)) · v11: S3 2800→3400(멀티터치 인간 실측 2,947 재측정)·텔레 적재 의심 임계 2000→4000 봉투 동기화(0130)·어드민 라벨 상수 연동 · v10: 맵변경 배율 ×2 곱(합산 ×4) → S2 이론상한 ×2·S3 2800·S7 252만·하드캡 4000/초 · v9: 맵별 투척 12종 · v8: gun 4→7
...
/** 인간 실측 최대 score/초 — 2026-09-25 제한 시간(v1.53) 판: 5손가락·맵 6곳·무기 9종·궁극기 7회로 455,526점/121.5초
 *  (타격 초당 15.1 · 간격 CV 1.1 · 텔레 정상, 사용자 확인 정상 플레이). 짧게 몰아치는 판이라 종전(2026-09-11, 44초 130,130점 = 2,947,
 *  그 전 단일 터치 1,267)보다 높다. 어드민 상세 라벨·S3 유도의 기준. */
export const HUMAN_SCORE_PER_SEC_OBSERVED = 3_748;
/** 인간 지속 apm 실측 상한(≥60s 표본 28, 2026-07-02 단일 터치 시절 관측 최대 879). 어드민 참조선·경고 전용 — 신호가 아니다.
 *  5손가락 세션(9/11 실측 114판)은 최대 1,129 까지 관측되므로 이 선 위라고 봇이 아니다. */
export const HUMAN_APM_SUSTAINED_OBSERVED = 880;
/** 최대 평균 score/초(S3 의심 플래그) = 인간 실측 최대 × 약 1.2(3,748 × 1.2 ≈ 4,498 → 4,500; v13, 사용자 결정 2026-09-25).
 *  1.2 인 이유: 실측 판(궁극기 7회)보다 궁극기를 더 쓴 상위 판 추정이 약 4,400 이라 종전 1.15(4,310)로는 또 걸린다.
 *  이력: v11 3,400(2,947 × 1.15) · v10 2,800 · v1.35 까지 1,400. 제한 시간 도입 2주 뒤(~10/9) 분포로 재측정.
 *  ⚠ 봉투 계층 불변식: `MAX_AVG_SCORE_PER_SEC`(5500, score-limits.ts 저장 하드상한) ≥ 이 값(4500, 의심
 *  플래그) ≥ `HUMAN_SCORE_PER_SEC_OBSERVED`(3748). 상한은 "저장 거부"용(정상 안 막게 넉넉), S3 는 "의심→리뷰"용이라
 *  서로 다른 계층 — 같게 두지 말 것. 상한을 낮추면 정상 고득점을 거부하고, S3 를 올리면 봇을 놓친다.
 *  텔레메트리 적재 RPC 의 suspicious 비율 임계는 S3 가 아니라 저장 상한(5500)과 같다(0133 — S8 은 "프로토콜상 불가능"만). */
export const SCORE_PER_SEC_MAX = 4_500;
/** S7 점수 하한 = S3 의 15분 봉투 상수(SCORE_PER_SEC_MAX × 900s = 4,050,000; v11 3,060,000·v10 2,520,000·v1.35 까지 1,260,000).
...
export const S7_LONG_SESSION_SCORE_FLOOR =
  SCORE_PER_SEC_MAX * (MAX_REASONABLE_DURATION_MS / 1000); // 4,050,000
/**
 * S11 제한 시간 점수 상한(v12) — 한 판은 최대 플레이 시간(+ 시간 종료 뒤 궁극기 마무리 여유 5초)을 넘을 수 없으니,
 * 그 시간 동안 S3 비율(v13 4,500/초)로 낼 수 있는 점수가 정상 판의 상한이다. 120초면 562,500(사람 실측 최대 3,748/초 × 125초 = 468,500;
 * v12 는 425,000 이라 실측 455,526 판이 걸렸다). 벽시계 duration 과 무관하게 점수만 보므로 탭 숨김으로 멈춘 시간은 영향이 없다.
 * 무플래그 위조 상한이 S7 하한(405만)에서 이 값으로 내려간다. 추가 시간은 클라 계산이라 검증할 수 없지만 최대 플레이 시간이 곧 봉투라 무관하다.
 */
