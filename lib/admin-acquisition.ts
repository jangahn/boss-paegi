...
import { readSupabaseRowsPaginated, requireSupabaseRows } from "@/lib/supabase-operation";
import { validateAdminRows } from "@/lib/admin-read-contract";

/**
 * 공유·유입 분석 — 하이브리드(v1.06): 오늘 = `analytics_rollup_rows_for_day`(단일 소스 RPC) 라이브,
 * 어제까지 = analytics_rollups(`day_kst < 오늘`만 — 이중계산 차단) 윈도우 합산(JS). 무식별 집계.
...
const liveAnalyticsToday = cache(async (): Promise<AnalyticsRollupRow[]> => {
  const admin = createAdminClient();
  const data = await requireSupabaseRows(
    "admin.acquisition.live_today",
    () => admin.rpc("analytics_rollup_rows_for_day", { p_day: kstDate(0) }),
  );
...
async function analyticsRollupRows(metrics: string[], window: StatWindow): Promise<AnalyticsRollupRow[]> {
  const admin = createAdminClient();
  ...
  const rollup = validateAdminRows<AnalyticsRollupRow>(
    "admin.acquisition.rollups",
    data,
    { day_kst: "date", ...METRIC_VALUE_SCHEMA },
  );
  const live = (await liveAnalyticsToday()).filter((r) => metrics.includes(r.metric));
  return [...rollup, ...live];
}
...
export async function getShareStats(window: StatWindow): Promise<ShareStats> {
