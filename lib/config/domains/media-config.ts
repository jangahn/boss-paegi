...
// 슬롯별 prefix 분리 — og 슬롯엔 'og/' 만, logo 슬롯엔 'logo/' 만 허용. 업로드 라우트도 이 정규식 재사용(단일 출처).
// 정식 UUID(8-4-4-4-12) — 코드베이스 표준과 일치(느슨한 [0-9a-f-]{36} 지양).
const UUID = "[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}";
export const OG_PATH_RE = new RegExp(`^og/\\d{6}/${UUID}\\.(png|jpg|webp)$`);
export const LOGO_PATH_RE = new RegExp(`^logo/\\d{6}/${UUID}\\.(png|jpg|webp)$`);
