...
/**
 * 점수대(5단계) + 롤에 맞는 시비 멘트 랜덤 선택.
 * 해당 단계 풀 + 직전 멘트 제외. role 미지정 시 boss, cfg 미지정 시 코드 기본값(경계·콘텐츠).
 */
export function randomTaunt(opts: {
  ...
  const { score, exclude, role = "boss", roleCfg, scoreCfg = SCORE_TIER_CONFIG_DEFAULT } = opts;
  const tier = Math.min(TIER_COUNT - 1, scoreTier(score, scoreCfg.thresholds));
  const pool = roleFrom(role, roleCfg).taunts[tier];
