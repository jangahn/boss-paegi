...
/** 작은 표시용 썸네일 변환(on-the-fly). 브라우저는 Accept 로 webp 자동(~40KB, 원본 ~600KB의 1/15). */
export const DOLL_THUMB_PX = 384;
const THUMB_TRANSFORM = { width: DOLL_THUMB_PX } as const;
...
export async function signedDollUrl(
  ...
  const { data, error } = await createAdminClient()
    ...
    .createSignedUrl(path, ttl, opts?.thumb ? { transform: THUMB_TRANSFORM } : undefined);
