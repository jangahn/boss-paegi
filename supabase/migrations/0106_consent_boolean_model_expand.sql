-- 0106_consent_boolean_model_expand.sql
-- 동의 모델 단순화(2026-08-21 운영 결정) 1/2 — expand.
-- ① 버전 무관 8-arg 동의 체인 신설(약관 개정 재동의 로직 폐지 — 동의는 했냐/안했냐 + 시각만).
--    구 10-arg/7-arg 체인은 구 앱 롤링 윈도우 동안 공존, 0107에서 제거.
-- ② 탈퇴 스크럽에 age_confirmed_at 포함 — 재가입은 신규가입과 완전히 동일한 동의 플로우.
-- ③ 재활성 함수의 reconsent_required 의존 제거(연령 포함 초기화로 대체).
-- ④ 기존 탈퇴 계정 백필 초기화.
...
begin
  ...
  -- 동의는 버전 무관 불리언 모델(2026-08-21) — 동의 시각만 기록한다.
...
as $function$
begin
  -- 버전 무관 동의(2026-08-21) — legal 발행과의 lock 커플링 불필요, user lock만.
...
-- ④ 기존 탈퇴 계정 초기화(동의·연령) — 재활성 시 신규 동일 플로우 보장.
update public.member_accounts ma
 ...
 where p.id = ma.user_id
   ...
   and (ma.age_confirmed_at is not null
     or ma.terms_agreed_at is not null
     or ma.privacy_agreed_at is not null);

-- ⑤ 생성 자격 동의 부품 폐기 — 증거가 하나라도 있으면 중단(무배선 확인된 0건 전제).
do $$
begin
  if exists (select 1 from public.generation_provider_acceptance_evidence) then
    raise exception 'generation_provider_acceptance_evidence_not_empty';
  end if;
end $$;
drop trigger if exists trg_generation_provider_evidence_immutable
  on public.generation_provider_acceptance_evidence;
drop function if exists public.bp_generation_provider_evidence_immutable();
drop function if exists public.record_generation_provider_acceptance(
  uuid, uuid, text, boolean, boolean, boolean);
drop function if exists public.generation_provider_acceptance_status(uuid);
drop function if exists public.prune_generation_provider_acceptance_evidence(integer);
drop table if exists public.generation_provider_acceptance_evidence;
