-- site_content 도메인 추가 — 소개/FAQ/SEO 메타를 app_settings 콘솔(어드민 편집·수정내역)로 관리.
-- app_settings key allowlist 2곳(테이블 CHECK + RPC)에 'site_content' 추가. additive·무중단.
...
alter table public.app_settings add constraint app_settings_key_check
  check (key in ('marketing_copy','role_content','score_config','badge_catalog','session_limits','growth_levers','site_content'));

-- 2) 원자 업데이트 RPC allowlist 확장(0025 본문 + site_content). CAS·감사·동시최초생성 정규화 동일.
...
declare
  ...
  v_new_version int;
begin
  if p_key not in ('marketing_copy','role_content','score_config','badge_catalog','session_limits','growth_levers','site_content') then
