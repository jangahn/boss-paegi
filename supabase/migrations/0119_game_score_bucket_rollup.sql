-- 0119: 게임 분석 롤업에 점수 구간 분포(game_score_1k) 추가 (v1.24, PR-A)
--
-- telemetry_rollup_rows_for_day(p_day) 가 cron(INSERT)·어드민 오늘 라이브(SELECT)의 단일 소스이므로
-- 여기에 차원을 더하면 하이브리드(오늘=raw·어제까지=롤업)가 자동 성립한다(0116 persona_games 와 같은 방식).
-- dim_key = floor(score/1000) — 단계 경계(score_config.thresholds, 어드민 편집)와 **무관한** 1,000점 버킷
-- 히스토그램. 어드민이 현재 경계로 버킷을 합산하므로 경계를 바꿔도 과거 판이 정확히 재분류된다
-- (그래서 경계는 1,000점 단위 제약). 대상 = 공개 판정 점수(review_status in ('registered','cleared'),
-- lib/score-visibility SCORE_VISIBLE_STATUS_SQL). 본문은 프로덕션 pg_get_functiondef 실측본(0116) + union 멤버 1개.
...
-- 과거 일자 백필 — 원본(scores)이 영구라 전 기간 정확. 신규 dim 만 insert(기존 행 무변경, 재실행 멱등).
