-- telemetry_ingest_envelope.pgtap.sql — 텔레메트리 적재 RPC 봉투 계약(0130, anti-abuse v11).
--
-- 단언: ① bp_ingest_telemetry_delta_core 의 suspicious 비율 임계 리터럴 = 저장 봉투(4,000점/초)·점수 캡 800만
--       ② 3,900점/초·2,500점/초(종전 2,000 드리프트 구간) 세션은 비의심, 4,100점/초 세션은 의심
...
select ok(
  ...
  'ingest suspicious ratio literal equals the storage envelope 4000/s (0130)'
);
select ok(
  pg_get_functiondef(
    'public.bp_ingest_telemetry_delta_core(uuid,uuid,boolean,jsonb)'::regprocedure
...
select is(
  (select (ack_ok->>'mode') || '/' || (ack_mid->>'mode') || '/' || (ack_over->>'mode') from ingest_env_ctx),
  'full/full/full',
  'all three envelope fixture deltas are ingested in full mode'
);
select is(
  (select t.suspicious from public.telemetry_sessions t join ingest_env_ctx c on t.id = c.s_ok),
  false,
  '3,900 pts/s session is not suspicious under the 4000/s envelope'
);
select is(
...
select is(
  ...
  true,
  '4,100 pts/s session above the envelope is suspicious'
);
