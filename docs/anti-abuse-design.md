...
| S10 | **ultScore 몰아넣기** | `ultScore>0 & 사용 0`(치명 ×3) · 사용횟수 > `hitCount/(ULT_HITS/2)+2` · `ultScore > 사용×8,000` | `validateGameplayStats` 가 못 막는 궁극기 필드 위조 폐쇄. |

S1·S2가 **직접 제출형(B)** 을 payload만으로 잡는 열쇠다. 위조자가 hitCount를 낮추면 avg-per-hit이 올라 S2에 걸리고, 높이면 S1에 걸린다(닫힌 박스). 봉투의 실질 바인딩은 **S3**(S2×S1 최대 ≈ 420×18 ≈ 7,560/s ≫ S3 1,400/s) — S2 교정으로 무기별 임계가 올라도 무플래그 위조 상한(1,400/s × 900s ≈ 126만점)은 불변이다. S3 는 비율이라 15분 초과 구간에선 봉투가 duration 에 비례해 늘어나는데, **S7**(v5)이 같은 126만 상수를 점수 하한으로 이어받아 상한이 duration 전 구간에서 불변. S2 의 역할은 총량이 아니라 "특정 무기에 점수를 몰아넣은 비정합 payload" 탐지.
