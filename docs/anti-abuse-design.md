...
| S3 | **평균 점수/초** `score/durationSec` | > 2,800 (v1.36, 종전 1,400) | 인간 검증 최대 1,267 은 ×2 시절 — 배포 2주 뒤 재측정. |
...
| S7 | **장기 세션 × 고득점** | `durationMs > 900,000`(15분) AND `score > 2,520,000`(=2,800/s × 900s; v1.35 까지 1,260,000) | 15분 초과 구간의 **봉투 연장** — S3 는 비율이라 duration 에 비례해 상한이 늘어나므로, 15분 봉투 상수를 점수 하한으로 걸어 무플래그 위조 상한 126만을 duration 무관 불변으로 유지. ≤15분 & >126만은 산술상 반드시 S3 발화라 상보적. ⚠ v4 까지는 duration 단독 조건이라 **세션 캡(30분) 도달 정상 제출을 100% 오탐**(캡 완주·탭 방치 — `clampForSubmit` 이 정확히 캡으로 안착, route 400 은 strict `>` 라 통과. 사례 `0528dbad`: 46초 플레이+방치, 1,194점) → v5 점수 하한 결합(2026-07-13). |
| S8 | **연결 텔레 suspicious** | 제출 시점 `telemetry.suspicious = true` | 오토클리커는 단조 판정이라 제출 시점에 이미 확정 → 치명 가중(×3). |
...
S1·S2가 **직접 제출형(B)** 을 payload만으로 잡는 열쇠다. 위조자가 hitCount를 낮추면 avg-per-hit이 올라 S2에 걸리고, 높이면 S1에 걸린다(닫힌 박스). S2는 v6부터 1타에도 적용되어 무기 분산으로 사라지는 구간이 없다. 봉투의 실질 바인딩은 **S3**(S2×S1 최대 ≈ 420×18 ≈ 7,560/s ≫ S3 1,400/s)이며, **S7**이 15분 이후에도 같은 126만 상수를 유지한다.

> **점수 봉투 계층(정합 불변식).** 세 값은 **의도적으로 다른 계층**이며 `MAX_AVG_SCORE_PER_SEC`(4000 — v1.36, 종전 2000, `score-limits.ts` — 저장 하드상한, 클라 클램프+서버 400 거부+DB 함수 0126) ≥ `SCORE_PER_SEC_MAX`(1400, S3 의심 플래그) ≥ 인간 검증 max(1267) 순서를 지킨다. **상한**은 정상 플레이를 절대 거부하지 않게 넉넉히 두고(넘으면 클램프/거부), **S3**는 그 아래에서 "의심→리뷰"로 flag한다. 같게 맞추지 말 것 — 상한을 낮추면 정상 고득점을 거부하고, S3를 올리면 봇을 놓친다. 텔레메트리는 클램프 **전** raw를 저장하므로 cron C1은 완주 텔레에서 `tscore(raw) ≥ 제출(clamp)`을 오탐하지 않도록 one-sided(§5.2).
