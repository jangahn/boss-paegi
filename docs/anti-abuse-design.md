...
> 상태: **구현 완료**(PR1~6, 2026-07-02) + **제출 봉투 완결 v6**(2026-07-29) · 작성: 2026-07-02 · 근거: 프로덕션 전수조사
...
2. **상한이 관대** — `MAX_AVG_SCORE_PER_SEC`(당시 2000, v1.36 부터 4000 — 저글링 합산 ×4 반영)은 검증된 인간 최고(1,267/초, ×2 시절)의 1.6배, 정상 최고점(406K)의 ~9배 위조를 통과시킴.
...
- **멀티터치 재측정(2026-09-11, v1.36 투척·맵변경 ×4 이후)**: 5손가락·투척 9종·맵 6·44초 정상 플레이 130,130점 = **2,947점/초**, 16.7타/초(apm 1,000), 간격 CV 0.77, 텔레↔점수 0%(어드민 정상 확인). 5손가락 세션 114판의 apm 최대 1,129 — 단일 터치 시절 지속 상한 880 은 멀티터치엔 참조선일 뿐 신호가 아니다. (당시 S3 의 기준.)
- **제한 시간 판 재측정(2026-09-25, v1.53 뒤)**: 5손가락·맵 6·무기 9종·궁극기 7회·121.5초 정상 플레이 455,526점 = **3,748점/초**, 15.1타/초(apm 908), 간격 CV 1.1, 텔레 정상(어드민 정상 확인). 최대 120초로 짧게 몰아치는 판이라 종전 최대보다 27% 높다. 이 값이 현행 S3 의 기준(`HUMAN_SCORE_PER_SEC_OBSERVED`, v13).
...
### 5.1 제출 시점 신호 (서버 — S1~S5·S7·S10 은 `body` payload 만으로 판정, S6/S8 은 연결 텔레메트리 링크/스냅샷 사용, S11 은 발행 설정의 최대 플레이 시간 사용)

| ID | 신호 | 플래그 조건 | 근거 |
|---|---|---|---|
| S1 | **지속 타격속도** `hitCount/durationSec` | ≥60초에서 > 18/s, 또는 임의 길이에서 > 25/s | 인간 지속 최대 ~15/s. REC2=23/s → 잡힘. REC1=57/s → 잡힘. |
| S2 | **무기별 점수 타당성** | 어떤 무기 w(≥1타)에서 `(weaponScores[w]−300)/weaponCounts[w] > 실효maxBase(w)×16×1.05`(v1.36: 콤보 ×4 × 무기변경 ×2 × 맵변경 ×2; v1.35 까지 ×8) | 타당 상한 = **실효 max base**(속도 배율 상한 반영: swipe ×2.0·throw ×2.2·grab fling strength+30, 고정무기는 strength) × 콤보캡4 × 다양성캡2. exact integer 집계이고 fresh 300도 정확히 1회 귀속되므로 표본 게이트가 필요 없다. v6에서 1~19타 분산 우회를 폐쇄했다. |
| S3 | **평균 점수/초** `score/durationSec` | > **4,500** (v13 = 인간 실측 3,748 × 약 1.2; v11 3,400·v1.36 2,800·종전 1,400) | 인간 실측 최대 3,748(2026-09-25 제한 시간 판). 봉투 5,500 > 4,500 > 3,748 유지. 1.2 배인 이유: 궁극기를 더 쓴 상위 판 추정 약 4,400. |
| S4 | **stats 누락/검증실패** | gameplayStats 없음 또는 `validateGameplayStats` 실패 | REC1의 "stats 없이 조용히 저장" 구멍을 닫음 → pending 처리. |
| S5 | **타격 간격 규칙성(CV)** | `intervalCV < 0.08` (hitCount ≥ 100) | 봇/매크로=거의 등간격(CV≈0), 인간은 편차가 큼. 클라 계측 미보정이라 보수적 0.08 시작(→§12). |
| S6 | **notable 무텔레메트리** | `score ≥ 300,000` AND 텔레 세션 링크 없음 | 리더보드 가치 점수는 텔레 정합 대상이어야 함 — 링크 자체를 뺀 위조 차단. |
| S7 | **장기 세션 × 고득점** | `durationMs > 900,000`(15분) AND `score > 4,050,000`(=4,500/s × 900s; v11 3,060,000·v10 2,520,000·v1.35 까지 1,260,000) | 15분 초과 구간의 **봉투 연장** — S3 는 비율이라 duration 에 비례해 상한이 늘어나므로, 15분 봉투 상수를 점수 하한으로 걸어 무플래그 위조 상한 126만을 duration 무관 불변으로 유지. ≤15분 & >126만은 산술상 반드시 S3 발화라 상보적. ⚠ v4 까지는 duration 단독 조건이라 **세션 캡(30분) 도달 정상 제출을 100% 오탐**(캡 완주·탭 방치 — `clampForSubmit` 이 정확히 캡으로 안착, route 400 은 strict `>` 라 통과. 사례 `0528dbad`: 46초 플레이+방치, 1,194점) → v5 점수 하한 결합(2026-07-13). |
| S8 | **연결 텔레 suspicious** | 제출 시점 `telemetry.suspicious = true` | 오토클리커는 단조 판정이라 제출 시점에 이미 확정 → 치명 가중(×3). suspicious 는 적재 RPC 가 서버에서 계산: raw 점수 > 초 × **4,000**(= 저장 상한, 0130 — 0126 이 2,000 을 못 올린 드리프트로 2,000~4,000 구간 정상 플레이가 전부 S8 오탐이었음) 또는 무기 타격 합계 ≠ hitCount(허용 max(10, 20%)). |
| S11 | **제한 시간 점수 상한**(v12, 2026-09-25 · v13 S3 연동) | `score > S3 × (최대 플레이 시간 + 5초)` — 최대 플레이 시간은 서버가 발행 설정(`session_limits.timeLimit.maxPlaySeconds`)에서 읽는다(120초면 v13 562,500 · v12 425,000) | 한 판은 최대 플레이 시간(+ 시간 종료 뒤 궁극기 마무리 여유)을 넘지 못하니 그동안 S3 비율로 낼 수 있는 점수가 정상 상한. 사람 실측 최대 3,748/초 × 125초 = 468,500(v12 에 걸린 실측 판 455,526 은 v13 에서 통과). 벽시계 duration 과 무관(탭 숨김으로 멈춘 시간 영향 없음). 무플래그 위조 상한 405만(S7 하한) → 56.25만. |
| S10 | **ultScore 몰아넣기** | `ultScore>0 & 사용 0`(치명 ×3) · 사용횟수 > `floor(hitCount/100 + max(hitCount−1,0)×0.1)` · `ultScore > 사용×8,000` | 첫 타 +0.01, 이후 매 타 switch bonus까지 최대 +0.11인 실제 게이지식의 보수적 수학 상한. 300ms switch cooldown은 무시하므로 정상보다 항상 같거나 넓지만, 과거 무조건 +2회 여유는 없다. |

S1·S2가 **직접 제출형(B)** 을 payload만으로 잡는 열쇠다. 위조자가 hitCount를 낮추면 avg-per-hit이 올라 S2에 걸리고, 높이면 S1에 걸린다(닫힌 박스). S2는 v6부터 1타에도 적용되어 무기 분산으로 사라지는 구간이 없다. 봉투의 실질 바인딩은 **S3**(S2×S1 최대 ≈ 704×18 ≈ 12,672/s ≫ S3 4,500/s)이며, **S7**이 15분 이후에도 같은 126만 상수를 유지한다.

> **점수 봉투 계층(정합 불변식).** 세 값은 **의도적으로 다른 계층**이며 `MAX_AVG_SCORE_PER_SEC`(5500 — v1.59, 종전 4000/2000, `score-limits.ts` — 저장 하드상한, 클라 클램프+서버 400 거부+DB 함수 0133) ≥ `SCORE_PER_SEC_MAX`(4500 — v13, 종전 3400/2800/1400, S3 의심 플래그) ≥ 인간 실측 max(`HUMAN_SCORE_PER_SEC_OBSERVED` 3748 — 2026-09-25 제한 시간 판, 종전 2947/1267) 순서를 지킨다. 텔레메트리 적재 RPC 의 suspicious 비율 임계는 S3 가 아니라 저장 상한과 같은 5500 이다(0133, `telemetry-ingest-envelope.test.ts` 계약). **상한**은 정상 플레이를 절대 거부하지 않게 넉넉히 두고(넘으면 클램프/거부), **S3**는 그 아래에서 "의심→리뷰"로 flag한다. 같게 맞추지 말 것 — 상한을 낮추면 정상 고득점을 거부하고, S3를 올리면 봇을 놓친다. 텔레메트리는 클램프 **전** raw를 저장하므로 cron C1은 완주 텔레에서 `tscore(raw) ≥ 제출(clamp)`을 오탐하지 않도록 one-sided(§5.2).

### 5.2 cron 백스톱 신호 (텔레메트리 확정 후 — `/api/ops/integrity-scan`)

텔레메트리는 delta 스트리밍이라 점수 제출 직후엔 미확정일 수 있음 → 정합 검사는 사후로.

**완주 게이트(C1·C1B 공통, v3/0054):** 정합 검사는 텔레가 **게임 끝까지 기록한 경우만** 수행한다 — `tscore>0` **그리고** `telemetry.end_reason ∈ {normal, time_limit, score_limit}`. `endSession(reason)`이 게임오버 사유로 텔레를 종료하므로 이 값이면 완주. `abandon/reload/hidden_timeout/null`은 조기 절단이라 duration·score가 부분값 → 정합 비교가 무의미(오탐). 근거: 텔레 duration은 collector `end()`에서 동결되는데 탭 30초+ 숨김 시 `hidden_timeout`으로 절단되는 반면, `scores.duration_ms`는 게임 벽시계(백그라운드 포함)라 정상 유저도 큰 "불일치"가 난다(전수조사 2026-07-08: 완주 텔레 184건 C1B 발화 0 / hidden_timeout 발화는 100%가 절단 아티팩트, 진짜 의심방향 0건). ⚠ 트레이드오프: 절단 텔레는 C1/C1B가 건너뛰므로 "탭 백그라운드로 텔레 절단 후 위조" 경로는 cron이 못 잡는다 — 단 위조 봉투는 제출시 **S3(4,500/s)**로 여전히 바인딩되고, 실제 절단-텔레 어뷰징은 관리자/오토클리커 판정이 담당(실측 REC2는 C1이 아니라 관리자 CONFIRMED_AUTOCLICKER로 차단).

| ID | 신호 | 플래그 조건 | 근거 |
|---|---|---|---|
| C1 | **텔레메트리↔점수 정합** (one-sided) | 완주 텔레에서 `(scores.score − telemetry.score)/score > 0.2` — **제출이 텔레(raw)보다 높을 때만** | REC2의 34배 위조(제출≫텔레) 차단. ⚠ 제출 점수는 클램프(min(raw, 초×2000)), 텔레는 raw 저장 → 완주 텔레에선 tscore ≥ 제출이 항상 성립. 대칭 `|·|` 은 이 클램프(정상)까지 오탐(예 `5c5e6435`: raw 171K→ceiling 130K, 32%)이라 **one-sided**(v4/0055). |
| C1B | **텔레메트리↔duration 정합** (대칭) | 완주 텔레에서 `|scores.duration_ms − telemetry.duration_ms|/telemetry.duration_ms > 0.2` | 제출 duration 위조 차단. duration 은 클램프가 없어(게임·텔레 둘 다 벽시계 종료시점) 위조가 양방향 가능 → **대칭 유지**. 절단 텔레는 완주 게이트로 제외. |
...
| C3 | (생략) **타격 간격 규칙성 (jitter/CV)** | 제출 S5와 동일 | gameStore에서 이미 계산한 CV를 제출 S5가 판정하고 telemetry `interval_cv`에도 기록한다. 동일 클라 소스의 cron 재판정은 독립 증거가 아니므로 중복 신호를 만들지 않는다. |

### 5.3 판정
- 제출 신호(S1–S11) 또는 cron 신호(C1·C1B·C2·C8) 중 **하나라도** 발화 → `review_status='pending'` + `leaderboard_hidden=true`.
- `abuse_score` = 발화 신호의 가중합(리뷰 큐의 위험도 칩 표시·우선 식별용, 판정 자체는 boolean OR. 큐 정렬은 최신 제출순 — 표시 날짜와 동일 키).
- 임계는 인간 최대의 1.5~3배 위에 있어 현재 데이터상 오탐 0이지만, 고주사율+터보마우스 등 회색지대 가능성 때문에 **하드 리젝이 아니라 flag→운영자 리뷰(가역)** 로 둔다.

### 5.4 PC 키보드 입력(v1.50) — 봉투 불변 원칙

키보드(스페이스·방향키)는 **별도 점수 경로가 아니라 가상 포인터 제스처**다(`game/input/KeyboardInput.ts` → PlayScene 의 기존 포인터 핸들러). 점수식·무기 쿨다운·`reportHit` 라이프사이클 펜스를 그대로 타므로 위 신호·임계는 바뀌지 않는다(`ANTI_ABUSE_RULES_VERSION` 유지). 지켜야 할 것:

- **OS 키 반복은 입력이 아니다.** 키를 누르고 있으면 OS 가 25~33회/초로 keydown 을 반복한다 — 그대로 받으면 S1(지속 18/s)·C2(apm 1,200)에 걸리고 간격이 등간격이라 S5(CV<0.08)의 봇 지문과 같아진다. 호출부(`useKeyboardControls`)가 `event.repeat` 를 버리고, 누르고 있기는 keydown/keyup 으로만 해석한다(비비탄총 자동 발사·꼬집기 늘리기·펜).
- **처리량(점/초)은 포인터로 낼 수 있는 최대 이하**(`KEYBOARD_TUNING`): 키보드는 호흡이 빨라(입력 기억 + 여운 캔슬) 빈도가 포인터 쿨다운을 넘을 수 있는 대신 그만큼 한 타 세기를 낮춘다. 싸대기 = 키 한 번에 왕복 2타, 첫 타 계수 2.0(28점)·역타와 연타 1.6(22점), 임팩트 간격 최소 120ms → 지속 22 × 8.3 = 183점/초 ≤ 포인터 최대 28 × 6.7(쿨다운 150ms) = 187점/초. 잡아던지기 = power 1.0(50점) × 시작 간격 300ms = 167점/초(250ms 는 벽 타격까지 더하면 v1.50 당시 S3 3,400 ÷ 16 = 212점/초에 여유 2%). 투척 = 정지 릴리즈(터치 탭 발사와 같은 길), 탭·투척 최소 간격 80ms(12.5회/초 < 사람 실측 지속 14.6회/초 < S1 18/s). 비비탄 180ms·꼬집기 흔들기 140ms·궁극기 연타 45ms 는 기존 쿨다운 상속, 비비탄은 키를 여러 개 눌러도 발사 줄기 하나. `keyboard-input.test.ts` 「봉투」가 무기별 이론 최대 처리량 × 최대 배율 16 × 90%(무기변경 배율 ×2 는 10초 창에 5종을 써야 유지된다) ≤ S3(v13 4,500) 를 고정한다.
- **누르고 있는 자동 동작은 등간격이 아니어야 한다(S5).** 꼬집기·펜의 스페이스는 누르는 동안 타격이 계속 나가는데, 등속으로 움직이면 간격 CV 가 0.04(실측)로 S5(`<0.08`, 100타 이상)에 걸려 정상 플레이가 검토 대기로 숨는다. 그래서 자동 움직임의 속도를 0.55~1.45배로 출렁이게 했다(손으로 그릴 때처럼) — 실측 CV 펜 0.34·꼬집기 0.20, `keyboard-input.test.ts` 가 `INTERVAL_CV_MIN × 1.5` 초과를 고정. 계측(`selectIntervalCV`)과 임계는 그대로다. 비비탄총 자동 발사는 포인터로 꾹 누르는 것과 같은 경로라 손대지 않았다(프로드 60일: 총 위주 100타 이상 53세션 중 CV<0.12 0건 — 실제 플레이는 무기 전환·프레임 변동으로 흩어진다).
- **궁극기 발동은 버튼과 같은 경로**(`handleUltimate` — 게이지 소비·`ultimateCount` 증가). 씬의 `triggerUltimate` 를 직접 부르면 S10 의 `ultScore>0 & 사용 0`(치명 ×3)이 된다.
- **기록**: 키보드 판은 pointer 이벤트가 없어 `max_touch` 0 이다. 입력 출처는 `telemetry_sessions.key_actions`(받아들여진 공격 동작 수, mig 0131)로 남고 어뷰징 상세 「키보드 동작」에 표시된다 — 신호가 아니라 검토 맥락.
...
- **jitter/CV 실기기 보정**: 계측·S5는 구현됐지만 고주사율·터보마우스 실기기 분포는 이 자동 QA 범위 밖이다. 현재 0.08의 보수적 임계를 유지하고 운영 분포를 본 뒤 조정한다.
...
- **S2 분포 모니터링 쿼리**(재보정·튜닝 변경 시 실행 — 무기별 fresh 차감 avg 분포):
  ...
  ```
- **S2 소량타격 우회 폐쇄(v6, 2026-07-29)**: `S2_MIN_HITS=1`. 9개 무기 각각 1..19타와 모든 9^1..9^4 무기 순열을 독립 득점 모델로 전수 검증해 이론 상한 이하 정상값은 통과하고 +1 초과는 발화함을 확인한다.
