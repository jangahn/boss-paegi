...
| S2 | **무기별 점수 타당성** | 어떤 무기 w(≥1타)에서 `(weaponScores[w]−300)/weaponCounts[w] > 실효maxBase(w)×8×1.05` | 타당 상한 = **실효 max base**(속도 배율 상한 반영: swipe ×2.0·throw ×2.2·grab fling strength+30, 고정무기는 strength) × 콤보캡4 × 다양성캡2. exact integer 집계이고 fresh 300도 정확히 1회 귀속되므로 표본 게이트가 필요 없다. v6에서 1~19타 분산 우회를 폐쇄했다. |
...
| S7 | **장기 세션 × 고득점** | `durationMs > 900,000`(15분) AND `score > 1,260,000`(=1,400/s × 900s) | 15분 초과 구간의 **봉투 연장** — S3 는 비율이라 duration 에 비례해 상한이 늘어나므로, 15분 봉투 상수를 점수 하한으로 걸어 무플래그 위조 상한 126만을 duration 무관 불변으로 유지. ≤15분 & >126만은 산술상 반드시 S3 발화라 상보적. ⚠ v4 까지는 duration 단독 조건이라 **세션 캡(30분) 도달 정상 제출을 100% 오탐**(캡 완주·탭 방치 — `clampForSubmit` 이 정확히 캡으로 안착, route 400 은 strict `>` 라 통과. 사례 `0528dbad`: 46초 플레이+방치, 1,194점) → v5 점수 하한 결합(2026-07-13). |
...
> **점수 봉투 계층(정합 불변식).** 세 값은 **의도적으로 다른 계층**이며 `MAX_AVG_SCORE_PER_SEC`(2000, `score-limits.ts` — 저장 하드상한, 클라 클램프+서버 400 거부) ≥ `SCORE_PER_SEC_MAX`(1400, S3 의심 플래그) ≥ 인간 검증 max(1267) 순서를 지킨다. **상한**은 정상 플레이를 절대 거부하지 않게 넉넉히 두고(넘으면 클램프/거부), **S3**는 그 아래에서 "의심→리뷰"로 flag한다. 같게 맞추지 말 것 — 상한을 낮추면 정상 고득점을 거부하고, S3를 올리면 봇을 놓친다. 텔레메트리는 클램프 **전** raw를 저장하므로 cron C1은 완주 텔레에서 `tscore(raw) ≥ 제출(clamp)`을 오탐하지 않도록 one-sided(§5.2).
