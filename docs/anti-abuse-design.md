...
- **멀티터치 재측정(2026-09-11, v1.36 투척·맵변경 ×4 이후)**: 5손가락·투척 9종·맵 6·44초 정상 플레이 130,130점 = **2,947점/초**, 16.7타/초(apm 1,000), 간격 CV 0.77, 텔레↔점수 0%(어드민 정상 확인). 5손가락 세션 114판의 apm 최대 1,129 — 단일 터치 시절 지속 상한 880 은 멀티터치엔 참조선일 뿐 신호가 아니다. 이 값이 S3 의 기준(`HUMAN_SCORE_PER_SEC_OBSERVED`).
...
| S3 | **평균 점수/초** `score/durationSec` | > **3,400** (v11 = 인간 실측 2,947 × 1.15; v1.36 2,800·종전 1,400) | 인간 실측 최대 2,947(2026-09-11 멀티터치). 봉투 4,000 > 3,400 > 2,947 유지. |
...
| S7 | **장기 세션 × 고득점** | `durationMs > 900,000`(15분) AND `score > 3,060,000`(=3,400/s × 900s; v10 2,520,000·v1.35 까지 1,260,000) | 15분 초과 구간의 **봉투 연장** — S3 는 비율이라 duration 에 비례해 상한이 늘어나므로, 15분 봉투 상수를 점수 하한으로 걸어 무플래그 위조 상한 126만을 duration 무관 불변으로 유지. ≤15분 & >126만은 산술상 반드시 S3 발화라 상보적. ⚠ v4 까지는 duration 단독 조건이라 **세션 캡(30분) 도달 정상 제출을 100% 오탐**(캡 완주·탭 방치 — `clampForSubmit` 이 정확히 캡으로 안착, route 400 은 strict `>` 라 통과. 사례 `0528dbad`: 46초 플레이+방치, 1,194점) → v5 점수 하한 결합(2026-07-13). |
...
| S11 | **제한 시간 점수 상한**(v12, 2026-09-25) | `score > 3,400 × (최대 플레이 시간 + 5초)` — 최대 플레이 시간은 서버가 발행 설정(`session_limits.timeLimit.maxPlaySeconds`)에서 읽는다(120초면 425,000) | 한 판은 최대 플레이 시간(+ 시간 종료 뒤 궁극기 마무리 여유)을 넘지 못하니 그동안 S3 비율로 낼 수 있는 점수가 정상 상한. 사람 실측 최대 2,947/초 × 125초 = 368,375. 벽시계 duration 과 무관(탭 숨김으로 멈춘 시간 영향 없음). 무플래그 위조 상한 306만(S7 하한) → 42.5만. |
