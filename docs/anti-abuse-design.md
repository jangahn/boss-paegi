...
| ID | 신호 | 플래그 조건 | 근거 |
|---|---|---|---|
| C1 | **텔레메트리↔점수 정합** | `|scores.score − telemetry.score|/score > 0.2`, 또는 연결 세션 누락 | REC2의 34배 불일치 차단. |
...
### 5.3 판정
- S1–S4 또는 C1–C2 중 **하나라도** 발화 → `review_status='pending'` + `leaderboard_hidden=true`.
