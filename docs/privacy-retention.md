...
## 방문일 기록 — 운영 대시보드 유저 퍼널·구성 (v1.17, 2026-09-03)

`user_visit_days(user_id, day_kst)` 는 상호작용·봇 게이트를 통과한 `/api/track`
방문 적재 때 서버가 세션 auth uid 로 남기는 **1행/유저/일** 기록이다(0117).
공유·유입 분석의 무식별 `analytics_*` 행과 결합하지 않으며 IP·UA·URL 등 다른
항목은 없다(uid·KST 일자뿐). 용도는 운영 통계(방문의 처음/다시 분해)에 한정한다.
FK 가 없어 계정 삭제 뒤에도 행이 남는다 — 과거 집계가 흔들리지 않게 하는 일별 동결과
같은 취지이며, 삭제된 uid 는 어떤 사람과도 다시 연결되지 않는다. 보유기간은 영구
(집계 전용)이고 익명→회원 이관 원장(`anon_data_reassignments`)으로 대표 계정에
접어서 읽는다. 개인정보처리방침에는 '서비스 이용 기록(방문 일자)' 범주로 고지한다
— v2 초안 발행 시 반영 대상.
