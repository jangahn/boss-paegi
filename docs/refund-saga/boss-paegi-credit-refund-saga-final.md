...
- **operational 컬럼 예외**(0063 column-level grant — H2 postflight·G-43(c)·eslint 룰 `allowedUpdateColumns` 와 동일 allowlist): orders `pg_status`·`raw`·`error_message` / member_accounts `email` / ai_generations `status`·`fail_reason`·`candidate_urls`·`fal_request_id`·`fal_request_ids`·`picked_doll_id`·`picked_index`·`cost_cents`·`role`·`gen_params`(gen_params 는 0070 이 operational 예외에 편입 — 생성 provenance jsonb). 금융/금융인접(orders.status·canceled_at·paid_at·payment_id·pg_tx_id·amount·credits·refunded_*·cancel-intent·receipt·gen_credits·credit_lot_id·consumed_at·refunded_at·version)은 direct grant 금지 — orders.status 전이는 `mark_order_failed`/`mark_order_canceled_unpaid`/`mark_paid_and_grant`/resolver 가 유일 경로. `refund_state`·consent 계열은 0063 에서 회수(각각 legacy·definer RPC 경유).
