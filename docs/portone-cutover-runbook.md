...
## 4. 심사용 계정 + 노출 설정 (콘솔)
1. 심사용 Google 계정 1개 준비 → 사이트 로그인 → 동의(만14세+약관+방침) 완료.
2. `/admin/content/growth_levers` → **"PG 심사용 계정 이메일"** 에 해당 이메일 등록 + 발행. (creditsEnabled 는 OFF 유지 — 일반 유저에겐 준비중 그대로.)
3. `/admin/content/site_content` → **사업자정보** 채워 발행(푸터 즉시 노출):
...
- KPN·토스페이·카카오페이 계약 완료 메일로 실 MID(KPN: MID+Secret OTP, 카카오페이: CA~ CID) 수령 → 콘솔에서 **실연동 채널 3개** 추가 → env 채널키 교체 + `PORTONE_WEBHOOK_SECRET` 실연동 값 교체.
- ⚠️ **카드사 심사 완료 전 실연동 결제는 실패가 정상** — 심사 완료 확인 후 creditsEnabled ON.
- Sentry: `payapp.*` 기반 알림 모니터를 `pay.*` 이벤트명으로 재설정(`pay.wh_grant_fail`·`pay.wh_amount_mismatch`·`pay.wh_paid_not_granted`·`pay.refund_commit_fail`·`pay.stale_payment_request`, 임계 1).
- reviewerEmails 비우기(심사 종료 시).
